...
//  Add Framework Automation Help as top-level module
...
cy.on('window:alert', cy.stub());`,
 ...
 'test-runner': entry({
   ...
   summary: 'Run Selenium/Playwright suites from TestUi and inspect logs and reports.',
   description:
     'In-app runner that starts Maven TestNG suites for the Java Selenium and Playwright projects, streams live logs, and surfaces Extent/Surefire reports plus screenshots. Browse project layout and suites in the separate Framework module.',
   concepts: [
     'TestNG suites',
     'Maven profiles',
     'Extent reports',
...
// UI: #runner-framework → playwright, then #runner-btn-start`,
   cypress: '// Prefer Selenium/Playwright Java projects for this runner.',
   suitableFor: 'Local suite execution with live logs; use Framework module for POM browse.',
 }),


 framework: entry({
   title: 'Framework',
   summary: 'Explore Java packages, TestNG suites, and the automation architecture diagram.',
   description:
     'Top-level module that scans automation/selenium-java and automation/playwright-java on disk. Browse Java project structure, open suite XML files that exist in folders, and review the framework architecture — independently of running tests.',
   concepts: [
     'Page Object Model (POM)',
     'TestNG suite XML layout',
     'Package layers (pages, tests, core, driver)',
     'Folder-based suite discovery',
     'Application vs module suites',
   ],
   techniques: [
     'Open Framework from the first Modules nav item',
     'Switch Selenium / Playwright project',
     'Browse Java Project Structure packages and class source',
     'Filter and open Test Suites under application/ and modules/',
     'Review Framework Architecture Diagram flow and counts',
   ],
   bestPractices: [
     'Add or remove suite XML under src/test/resources/suites — the list updates automatically',
     'Use stable package folders under com.testui.*',
     'Prefer module smoke suites when learning the layout',
   ],
   selenium: `// UI: #nav-link-framework → #framework-tab-java / #framework-tab-suites / #framework-tab-diagram
// Suites live under:
// automation/selenium-java/src/test/resources/suites/application/
// automation/selenium-java/src/test/resources/suites/modules/<module>/`,
   playwright: `// Same Framework module; select Playwright in #framework-select
// Suites: automation/playwright-java/src/test/resources/suites/`,
   cypress: '// N/A — this module documents the Java TestNG projects.',
   suitableFor: 'Onboarding to the automation codebase without executing Maven.',
 }),
